...
interface LibraryProps {
  onSelectTopic: (topic: { title: string; context: string }) => void;
}

export const Library: React.FC<LibraryProps> = ({ onSelectTopic }) => {
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <AnimatePresence mode="wait">
          {activeSection === 'defense' && (
            ...
            >
              {DEFENSE_TECHNIQUES.map((tech, i) => (
                <div key={i} className="p-6 brutal-border bg-white/5 hover:bg-white/10 transition-colors">
                  <h3 className="font-display text-2xl uppercase text-neon-green mb-2">{tech.title}</h3>
                  <p className="font-sans text-sm opacity-70 mb-4">{tech.description}</p>
                  <div className="bg-white/10 p-3 border-l-4 border-white/30">
                    <span className="font-mono text-[10px] uppercase opacity-50 block mb-1">实战场景</span>
                    <p className="text-sm italic">“{tech.scenario}”</p>
                  </div>
                </div>
              ))}
            </motion.div>
            ...
            >
              {EMOTION_TIPS.map((tip, i) => (
                <div key={i} className="p-8 brutal-border bg-white/5 flex gap-6 items-start">
                  ...
                  <div>
                    <h3 className="font-display text-3xl uppercase mb-2">{tip.title}</h3>
                    <p className="font-sans text-lg opacity-80 leading-relaxed">{tip.tip}</p>
                  </div>
                </div>
              ))}
            </motion.div>
              ...
              {DEBATE_TOPICS.map((topic, i) => (
                ...
                >
                  <div className="flex-1">
                    <h3 className="font-display text-xl uppercase mb-1">{topic.title}</h3>
                    <p className="font-sans text-xs opacity-70 group-hover:opacity-100">{topic.context}</p>
                  </div>
